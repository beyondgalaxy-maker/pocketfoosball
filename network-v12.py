...
OUT=ROOT/'network-check-v12';OUT.mkdir(exist_ok=True)
 ...
 with sync_playwright() as pw:
  ...
  try:
   for p in [h,j]:
    ...
    p.wait_for_function('window.touchline?.version==="12.0.0"')
   ...
   bad=h.evaluate('touchline.online.session.invalid');j.evaluate('touchline.online.send({t:"input",v:12,seq:999999,epoch:touchline.online.session.epoch,rows:[[0,999]]})');j.wait_for_timeout(100)
