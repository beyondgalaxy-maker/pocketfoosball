...
class DataConnection extends Emitter{
 constructor(owner,remote,incoming=false,metadata={}){super();this.metadata=metadata;this.owner=owner;this.peer=remote;this.open=false;this.dataChannel=null;this.pc=new RTCPeerConnection({iceServers:[]});this.pending=[];this.dead=false;
...
class Peer extends Emitter{
 constructor(id){super();this.id=id||'guest-'+Array.from(crypto.getRandomValues(new Uint8Array(12)),x=>x.toString(16)).join('');this.destroyed=false;this.connections={};window.__testPeer=this;window.__registerRTC(this.id).then(()=>{this.open=true;this.emit('open',this.id);});}
 signal(to,payload){return window.__signalRTC({from:this.id,to,...payload});}
 connect(to,options={}){const c=new DataConnection(this,to,false,options.metadata);this.connections[to]=c;(async()=>{const offer=await c.pc.createOffer();await c.pc.setLocalDescription(offer);await this.signal(to,{kind:'offer',sdp:c.pc.localDescription.toJSON(),metadata:options.metadata});})().catch(e=>this.emit('error',e));return c;}
 async receive(m){if(this.destroyed)return;let c=this.connections[m.from];if(m.kind==='offer'){c=new DataConnection(this,m.from,true,m.metadata);this.connections[m.from]=c;this.emit('connection',c);await c.remoteDescription(m.sdp);const answer=await c.pc.createAnswer();await c.pc.setLocalDescription(answer);await this.signal(m.from,{kind:'answer',sdp:c.pc.localDescription.toJSON()});}else if(m.kind==='answer')await c.remoteDescription(m.sdp);else if(m.kind==='candidate'){if(c)await c.candidate(m.candidate);else{(this.early??=[]).push(m);return;}}if(this.early?.length&&c){const queue=this.early;this.early=[];for(const item of queue)await this.receive(item);}}
