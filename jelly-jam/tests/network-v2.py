...
ROOT=Path(__file__).resolve().parents[1];OUT=ROOT/'tests/v2';OUT.mkdir(exist_ok=True)
...
try:
 with sync_playwright() as p:
  ...
  x=h.evaluate('JellyJam.state.players[1].x');g.keyboard.down('ArrowRight');g.wait_for_timeout(430);g.keyboard.up('ArrowRight');g.wait_for_timeout(180);check('Remote movement and snapshot agreement',h.evaluate('JellyJam.state.players[1].x')>x+55 and abs(h.evaluate('JellyJam.state.players[1].x')-g.evaluate('JellyJam.state.players[1].x'))<15)
  ...
  g.click('#restart');g.wait_for_timeout(160);check('Guest restart remains host-authoritative',g.evaluate('JellyJam.state.level')==13 and g.evaluate('JellyJam.state.t')<1)
  ...
  _,g2=make(gc,mobile=True);g2.click('#online');g2.fill('#roomInput',code);g2.click('#joinForm button');wait(g2,'JellyJam.connected');wait(g2,f'JellyJam.state.level==={level}');wait(g2,'JellyJam.paused');check('Reconnect preserves host level and pauses safely',h.evaluate('JellyJam.paused'));ready_pair(h,g2)
  # Disconnect the underlying channel (not a game handler) to test automatic retry.
  if not CLOUD:
   g2.evaluate('Object.values(__testPeer.connections).forEach(c=>c.close())');wait(h,'!JellyJam.connected');wait(g2,'JellyJam.connected',25);wait(h,'JellyJam.connected');wait(g2,'JellyJam.paused');ready_pair(h,g2);check('Dropped channel reconnects automatically',True)
