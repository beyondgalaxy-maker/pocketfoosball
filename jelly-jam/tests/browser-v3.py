...
try:
 with sync_playwright() as p:
  ...
  page.click('#levels');check('Six chapter tabs',page.locator('[data-chapter]').count()==6)
  all_ids=[]
  ...
  check('60 distinct selectable levels',len(set(all_ids))==60)
  page.screenshot(path=str(OUT/'map-expert.png'));page.click('[data-chapter="3"]');page.click('[data-level="39"]');page.click('#playSelected');check('Atlas starts selected level',page.evaluate('JellyJam.state.level')==39)
  ...
  for width,height in [(390,844),(844,390)]:
   ...
   m.screenshot(path=str(OUT/f'mobile-{width}.png'))
   m.click('#levels');m.click('[data-chapter="4"]');m.click('[data-level="42"]');m.click('#playSelected')
   rb=m.locator('[data-p="1"][data-key="right"]').bounding_box()
   cdp.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':rb['x']+rb['width']/2,'y':rb['y']+rb['height']/2,'id':7}]})
   m.wait_for_function('JellyJam.state.players[1].x>382');cdp.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]});m.wait_for_timeout(140)
   ab=m.locator('[data-p="1"][data-key="action"]').bounding_box()
   check(f'{width} interaction button is reachable',ab['width']>=44 and ab['y']+ab['height']<=height)
   cdp.send('Input.dispatchTouchEvent',{'type':'touchStart','touchPoints':[{'x':ab['x']+ab['width']/2,'y':ab['y']+ab['height']/2,'id':8}]});m.wait_for_timeout(70);cdp.send('Input.dispatchTouchEvent',{'type':'touchEnd','touchPoints':[]});m.wait_for_timeout(90)
   check(f'{width} native touch rotates mirror and lights receiver',m.evaluate('JellyJam.state.mirrorAngles[0]===0 && JellyJam.state.lit[0]'))
   m.screenshot(path=str(OUT/f'mobile-mirror-{width}.png'));context.close()

  # Teaching and interaction must work using actual buttons and keyboard, not test state edits.
  page.click('#brand');page.click('#leave');page.click('#adventureMap');page.click('[data-chapter="4"]');page.click('[data-level="42"]');page.click('#playSelected')
  check('Mirror controls taught in the level', 'E / Enter' in page.inner_text('#lessonText') or 'Peach: E' in page.inner_text('#lessonText'))
  page.keyboard.down('KeyD');page.wait_for_function('JellyJam.state.players[0].x>382');page.keyboard.up('KeyD');page.wait_for_timeout(150);page.keyboard.press('KeyE');page.wait_for_timeout(80)
  check('Keyboard rotates a nearby mirror',page.evaluate('JellyJam.state.mirrorAngles[0]===0'))
  check('Reflected light actually opens receiver door',page.evaluate('JellyJam.state.lit[0] && JellyJam.state.doors[0]'))
  page.screenshot(path=str(OUT/'mirror-school.png'))
  page.click('#levels');page.click('[data-chapter="5"]');page.click('[data-level="58"]');page.click('#playSelected')
  check('Large puzzle overview available',page.locator('#overview').is_visible())
  page.screenshot(path=str(OUT/'observatory.png'));before_overview=page.get_attribute('#overview','aria-pressed');page.click('#overview');check('Overview toggle responds',page.get_attribute('#overview','aria-pressed')!=before_overview)
  page.click('#lessonGuide');check('In-level field guide explains current stage', 'THREE-ROOM CHAIN' in page.inner_text('#modalContent'));page.click('#gotIt')
  page.click('#levels');page.click('[data-chapter="4"]');page.click('[data-level="40"]');page.click('#playSelected')
  page.keyboard.down('KeyD');page.wait_for_function('JellyJam.state.players[0].x>325');page.keyboard.up('KeyD');page.wait_for_function('JellyJam.state.players[0].y<300')
  check('Fan lifts through ordinary movement only',page.evaluate('JellyJam.state.players[0].y<300'))
  page.screenshot(path=str(OUT/'fan-school.png'))
