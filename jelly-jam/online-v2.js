...
(function(root){
'use strict';
const VERSION=2,ALPHABET='ABCDEFGHJKLMNPQRSTUVWXYZ23456789';
...
class Room{
 constructor(handlers={}){this.handlers=handlers;this.peer=null;this.conn=null;this.role='';this.code='';this.connected=false;this.generation=0;this.lastSeen=0;this.lastPulse=Date.now();this.remoteHidden=false;this.everConnected=false;this.retries=0;this.epoch='';this.seat='';this.timers=new Set();try{this.token=sessionStorage.getItem('jj2-seat')||random();sessionStorage.setItem('jj2-seat',this.token);}catch{this.token=random();}this.heartbeat=setInterval(()=>this.pulse(),2000);}
 ...
 makePeer(){if(!this.role)return;const opts={debug:0,...(root.JELLY_PEER_OPTIONS||{})};const p=this.role==='host'?new root.Peer('jellyjam-v2-'+this.code,opts):new root.Peer(undefined,opts);this.peer=p;
 ...
 dial(){if(this.role!=='guest'||!this.peer||this.peer.destroyed||this.peer.disconnected||this.conn)return;
  try{this.attach(this.peer.connect('jellyjam-v2-'+this.code,{reliable:true,serialization:'json',metadata:{game:'jellyjam',v:VERSION,seat:this.token}}));}catch{this.retry();}}
 ...
 attach(c){this.conn=c;let welcomed=false;const timer=this.later(()=>{if(this.conn===c&&!welcomed)this.drop('Reconnect in progress. Keep both tabs open.');},12000);
  ...
  c.on('data',d=>{if(this.conn!==c||!d||typeof d!=='object'||typeof d.type!=='string')return;this.lastSeen=Date.now();
   ...
   if(d.type==='version'){this.event('error','Both buddies must refresh to Jelly Jam 2.');this.close();return;}
